...
void dcost(float eps ,float a, float b, float bias, float *da, float *db, float *dbias)
{
    float c = cost(a, b, bias);
    *da = (cost(a+eps, b, bias) - c) / eps;
    *db = (cost(a, b+eps, bias) - c) / eps;
    *dbias = (cost(a, b, bias+eps) - c) / eps;
}

float gcost(float a, float b, float bias, float *da, float *db, float *dbias){
    *da = 0;
    *db = 0;
    *dbias = 0;

    //iterating over the sample data for calculating cost.
    size_t n = TRAIN_COUNT;
    for(size_t i=0; i<n; i++){
        //calculate the ai,bi and their operation will output zi
        float xi = train_or[i][0];
        float yi = train_or[i][1];
        float zi = train_or[i][2];
        //applying activation
        //axi + yi + bias, passed through sigmoid for limiting value within 0 to 1 range
        float ai = sigmoidf(xi*a + yi*b + bias); 
        //the derivative of the a
        *da += 2*(ai - zi)*ai*(1 - ai)*xi;
        *db += 2*(ai - zi)*ai*(1 - ai)*yi;
        *dbias += 2*(ai - zi)*ai*(1 - ai);
    }
    //average sum
    *da /= n;
    *db /= n;
...
#if 1
        float eps = 1e-3;
        dcost(eps, w1, w2, bias, &da, &db, &dbias);
        
        
#else
        gcost(w1,w2,bias,&da, &db, &dbias);
#endif
        w1 -= rate * da;
        w2 -= rate * db;
        bias -= rate * dbias;
